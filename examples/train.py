...
def run_epoch(algorithm, dataset, general_logger, epoch, config, train, val_dataset=None, test_datasets=None):
    if dataset['verbose']:
        ...
        if train and config.evaluate_steps is not None and batch_idx % config.evaluate_steps == 0:
            ...
            if test_datasets is not None:
                for test_dataset in test_datasets:
                    metric_split, test_y_pred = run_epoch(algorithm, test_dataset, general_logger, epoch, config, train=False)
                    save_pred_if_needed(test_y_pred, test_dataset, epoch, config, is_best)
            algorithm.train()
            torch.set_grad_enabled(True)
...
def train(algorithm, datasets, general_logger, config, epoch_offset):
    ...
    for epoch in range(epoch_offset, config.n_epochs):
        ...
        for split in additional_splits:
            metric_split, y_pred = run_epoch(algorithm, datasets[split], general_logger, epoch, config, train=False)
