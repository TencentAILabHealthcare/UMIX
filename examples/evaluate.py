...
def evaluate_benchmark(
    ...
    def get_prediction_file(
        ...
        for file in os.listdir(predictions_dir):
            if file.startswith(run_id) and (
                file.endswith("best_pred.csv") or file.endswith("best_pred.pth")
            ):
                return file
    ...
    def get_metrics(dataset_name: str) -> List[str]:
        if "amazon" == dataset_name:
            ...
            return ["acc_avg"]
        elif "waterbirds" == dataset_name:
            return ["acc_wg", "acc_avg", "acc_y:landbird_background:land","acc_y:landbird_background:water", "acc_y:waterbird_background:land", "acc_y:waterbird_background:water"]
        elif "celebA" == dataset_name:
            return ["acc_wg", "acc_avg", "acc_y:notblond_male:0", "acc_y:notblond_male:1", "acc_y:blond_male:0", "acc_y:blond_male:1"]
        else:
